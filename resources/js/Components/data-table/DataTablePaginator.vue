...
const props = defineProps({
    table: {
        ...
        validator: (value) =>
            value &&
            typeof value.previousPage === 'function' &&
...
<template>
    <div class="flex mt-4 items-center justify-between px-2">
        <div class="flex-1 text-sm text-muted-foreground">
            {{ $t("app.showing_records", { records: table.getFilteredRowModel().rows.length }) }}
        </div>
        <div class="flex items-center space-x-6 lg:space-x-8">
            <div class="flex items-center space-x-2">
                <p class="text-sm font-medium">
                    {{ $t("app.records_per_page") }}
                </p>
